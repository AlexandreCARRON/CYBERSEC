...
ALLOWED_ACTIONS = frozenset({"http_observe", "ai_assess"})
...
def engagement_from_dict(data: Dict[str, Any]) -> Engagement:
    ...
    if data.get("schema_version") != 1:
        raise EngagementError("schema_version doit valoir 1.")
    unknown = set(data) - {
        "schema_version",
        "engagement_id",
        "title",
    ...
    starts_at = _parse_datetime(data.get("starts_at"), "starts_at")
    ...
    for item in raw_scope:
        if not isinstance(item, dict) or not {"origin", "allowed_actions"} <= set(item):
            raise EngagementError("Chaque entrée de scope doit définir origin et allowed_actions.")
        if set(item) - {"origin", "allowed_actions", "auth"}:
        ...
        if not isinstance(actions, list) or not actions or not all(isinstance(v, str) for v in actions):
            raise EngagementError("allowed_actions doit être une liste non vide.")
        action_set = frozenset(actions)
        invalid = action_set - ALLOWED_ACTIONS
        if invalid:
            raise EngagementError("Actions inconnues: %s" % ", ".join(sorted(invalid)))
        if origin in seen:
        ...
        seen.add(origin)
        entries.append(ScopeEntry(origin=origin, allowed_actions=action_set, auth=auth))
...
def _network_allowed(address: ipaddress._BaseAddress, network_policy: str) -> bool:
...
def authorize(
    ...
    resolver: Callable[[str, int], Iterable[ipaddress._BaseAddress]] = _resolved_addresses,
) -> Tuple[str, str, ScopeEntry]:
    ...
    if action not in entry.allowed_actions:
        raise EngagementError("L'action n'est pas autorisée pour cette cible.")
    parsed = urlsplit(target)
    port = parsed.port or (80 if parsed.scheme == "http" else 443)
    addresses = tuple(resolver(parsed.hostname or "", port))
    if not addresses or not all(_network_allowed(address, engagement.network_policy) for address in addresses):
        raise EngagementError("La résolution DNS viole la politique réseau de l'engagement.")
    return origin, target, entry
